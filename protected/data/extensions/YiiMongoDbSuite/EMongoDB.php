	...
	public $safeFlag = false;
                ...
                if($this->_mongoConnection === null)
                {
                        try
                        {
                                ...
                                if($this->persistentConnection !== false)
                                        $this->_mongoConnection = new Client($this->connectionString, array(
        ...
        public function getDbInstance()
        {
                if($this->_mongoDb === null)
                  return $this->_mongoDb = $this->getConnection()->selectDatabase($this->dbName);
                elseif(class_exists('MongoHelper') && MongoHelper::readPrefSecondary()) {
                        $this->setReadPreference('secondaryPreferred');
        ...
        public function setReadPreference($pref){
                $this->_mongoDb  = $this->getConnection()->selectDatabase($this->dbName, [
                        'readPreference' => new MongoDB\Driver\ReadPreference($pref),
                ]);       
        }
        /**
        ...
        {
                $this->_mongoDb = $this->getConnection()->selectDatabase($name);
        }
