...
abstract class EMongoDocument extends DBDocumentPHP7
{
	...
	public function setAttributes($values, $safeOnly=true)
	{
		...
		$values = MongoCollectionHelper::toLegacy($values);
		if(!is_array($values))
			return;
	...
	public function init()
	{
		...
		if($this->ensureIndexes && !isset(self::$_indexes[$this->getCollectionName()]))
		{
			...
			$indexInfo = array();
			foreach ($this->getCollection()->listIndexes() as $index){
				array_push($indexInfo,$index);
			};
	...
	public function insert(array $attributes=null)
	{
		...
		if($this->beforeSave())
		{
			...
			$result = $this->getCollection()->insertOne(MongoCollectionHelper::fromLegacy($rawData), [
		...
		if($this->beforeSave())
		{
			...
			if($modify)
			{
				...
				$result = $this->getCollection()->updateOne(
					MongoCollectionHelper::fromLegacy(['_id' => $this->_id]),
					['$set' => MongoCollectionHelper::fromLegacy($rawData)],
					array(
						'fsync'=>$this->getFsyncFlag(),
						'w'=>$this->getSafeFlag(),
						'multiple'=>false
					...
					// 	$result = $this->getCollection()->replaceOne($rawData, array(
					// 	'fsync'=>$this->getFsyncFlag(),
					// 	'w' =>$this->getSafeFlag()
					// ));
					$id = MongoCollectionHelper::ensureDocumentHasMongoId($rawData);
					$options = array(
							'fsync'=>$this->getFsyncFlag(),
							'w' =>$this->getSafeFlag()
					);
					$result = $this->collection->replaceOne(
						MongoCollectionHelper::fromLegacy(['_id' => $id]),
						MongoCollectionHelper::fromLegacy($rawData),
						MongoCollectionHelper::convertWriteConcernOptions($options)
					);
	...
	/**
	 *  Works just like update but without beforesave and aftersave 
	 */
	public function saveAttributes(array $attributes=null, $modify = false)
	{
		if($this->getIsNewRecord())
			throw new CDbException(Yii::t('yii','The EMongoDocument cannot be updated because it is new.'));
			...
			if($modify)
			{
				...
				$result = $this->getCollection()->updateOne(
					...
					$result = $this->getCollection()->save(MongoCollectionHelper::fromLegacy($rawData), array(
						'fsync'=>$this->getFsyncFlag(),
						'w'=>$this->getSafeFlag()
					));
			}
	...
	public function updateAll($modifier, $criteria=null) {
		...
		if($modifier->canApply === true)
		{
			$this->applyScopes($criteria);
				$result = $this->getCollection()->updateMany(MongoCollectionHelper::fromLegacy($criteria->getConditions()), $modifier->getModifiers(), array(
					'fsync'=>$this->getFsyncFlag(),
					'w'=>$this->getSafeFlag(),
					'upsert'=>false,
					'multiple'=>true
	...
	public function deleteByPk($pk, $criteria=null)
	{
		...
		$criteria->mergeWith($this->createPkCriteria($pk));
			...
			$result = $this->getCollection()->deleteOne(MongoCollectionHelper::fromLegacy($criteria->getConditions()), array(
				'justOne'=>true,
				'fsync'=>$this->getFsyncFlag(),
				'w'=>$this->getSafeFlag()
			));
	...
	public function find($criteria=null)
	{
		...
		if($this->beforeFind())
		{
			...
			$doc = $this->getCollection()->findOne(
				MongoCollectionHelper::fromLegacy($criteria->getConditions()), 
				['projection'=>$criteria->getSelect(true)]
			);
	...
	public function findAll($criteria=null)
	{
		...
		{
			$this->applyScopes($criteria);
			$sortVariable =  $criteria->getSort();
			$getLimit = $criteria->getLimit();
			$getOffset = $criteria->getOffset();
			$getSelect = $criteria->getSelect(true);
	...
	public function count($criteria=null)
	{
		...
		return $this->getCollection()->count(MongoCollectionHelper::fromLegacy($criteria->getConditions()));
	}

	public function distinct($attribute, $criteria=null)
	{
		Yii::trace(get_class($this).'.count()','ext.MongoDb.EMongoDocument');

 		$this->applyScopes($criteria);

 		return $this->getCollection()->distinct($attribute, MongoCollectionHelper::fromLegacy($criteria->getConditions()));
	}
	...
	{	
		Yii::trace(get_class($this).'.deleteByPk()','ext.MongoDb.EMongoDocument');
		$this->applyScopes($criteria);
			$result = $this->getCollection()->deleteMany(MongoCollectionHelper::fromLegacy($criteria->getConditions()), array(
				'justOne'=>false,
				'fsync'=>$this->getFsyncFlag(),
				'w'=>$this->getSafeFlag()
			));
	...
	{
		return (new MongoAggregator)->setCollection($this->getCollection());
	}
